...
float4 main(PS_INPUT input) : SV_TARGET
{
    ...
    float4 ambient = txDiffuse.Sample(samLinear, input.Tex) * 0.8f;
    ...
    // SPECULAR
    float3 vToCam = normalize(input.Pos.xyz - input.worldPos.xyz);
    float3 vLight = normalize(input.worldPos.xyz - lightPos.xyz);
    float3 vReflect = reflect(-vLight, input.Norm);
    float4 fSpecDot = dot(vReflect, vToCam);
    fSpecDot = pow(fSpecDot, 128); // second param is power of specular
    finalColor += fSpecDot * 1.0f * lightRatio;
